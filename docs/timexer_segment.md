...
Forward and backward are NOT graph-captured, and the step deliberately does not try. Capturing them requires resident inputs and a private memory pool holding the step's whole transient working set, which at batch 256 is ~15 GB of activations on top of the warmup allocations already retained in the global pool: benchmark job 5116 died with `CUDA out of memory. Tried to allocate 282.00 MiB` while capturing, on an otherwise idle 32 GB device. The accounting says the trade was bad anyway - the step issues ~800-900 kernels, so host launch latency is 4-8 ms of a 302 ms step, at most 2.6%, against 15 GB of unreusable reservation and an OOM risk in a 12 h run. What the step does keep: no scalar crosses to the host inside a step (the objective's finiteness is accumulated on device as an indicator and read once per report interval with the interval's mean loss), and the optimizer step remains captured inside NorMuon.
...
- `timexer_segment_benchmark_kernels` — `milliseconds`: one backbone layer at the real shapes,
  per class of kernel (RMSNorm, QK RMSNorm, QKV projection, rotary rotation, causal SDPA,
