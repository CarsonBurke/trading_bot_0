...
impl Env {
    pub(super) fn get_next_obs(&self) -> (Vec<f32>, Vec<f32>) {
        ...
        // === Per-ticker observations (PER_TICKER_STATIC_OBS = 19) ===
        ...
        for (perm_idx, &real_idx) in self.ticker_perm.iter().enumerate() {
            ...
            static_obs.push(self.account.positions[real_idx].appreciation(self.prices[real_idx][absolute_step]).clamp(-1.0, 1.0) as f32);
            ...
            static_obs.push(self.position_open_step[real_idx].map(|s| (absolute_step.saturating_sub(s) as f64 / 500.0).min(1.0)).unwrap_or(0.0) as f32);
            static_obs.push(self.target_weights[real_idx].clamp(0.0, 1.0) as f32);
            // Multi-scale momentum (4)
            static_obs.push(m.mom_5[absolute_step] as f32);
            static_obs.push(((self.prices[real_idx][absolute_step] / self.prices[real_idx][absolute_step.saturating_sub(20)] - 1.0).clamp(-0.5, 0.5)) as f32);
            ...
            // Oscillators (4)
            static_obs.push((m.rsi[absolute_step] * 2.0 - 1.0) as f32); // center at 0
            static_obs.push(m.range_pos[absolute_step] as f32);
            static_obs.push((m.stoch_k[absolute_step] * 2.0 - 1.0) as f32);
            static_obs.push((m.zscore[absolute_step] / 3.0) as f32); // normalize to ~[-1,1]
            // Trend (1)
