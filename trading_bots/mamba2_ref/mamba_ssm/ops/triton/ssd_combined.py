...
def rearrange_and_update_stride(tensor, pattern=None, dim=2):
    ...
    # if not, make the source contiguous first (preserves channel-last layout after rearrange)
    # then fall back to .contiguous() only if that doesn't help
    ...
    # Make source contiguous before rearranging to preserve channel-last layout
    if pattern is not None:
        tensor_rearranged = rearrange(tensor.contiguous(), pattern)
        if tensor_rearranged.stride(dim) % 8 == 0:
            return tensor_rearranged
...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def backward(ctx, dout, *args):
        ...
        dxBC_given = rearrange_and_update_stride(dxBC_given, "b s d -> b d s")
        dxBC_given_update, dweight, dbias, *_ = causal_conv1d_bwd_function(
            ...
            rearrange_and_update_stride(dxBC, "b s d -> b d s"), seq_idx, None, None, dxBC_given, False, ctx.activation in ["silu", "swish"]
        )
