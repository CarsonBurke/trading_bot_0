...
use crate::{constants::agent::TARGET_AGENT_COUNT, types::MappedHistorical, utils::{convert_historical, ema, ema_diff_percent, get_macd, get_rsi_percents, get_rsi_values}};
...
pub fn create_mapped_indicators(mapped_data: &MappedHistorical) -> HashMap<String, Indicators> {
    ...
    for (ticker, bars) in mapped_data.iter() {
        ...
        indicators.insert(ticker.to_string(), create_indicators(data));
    }
...
fn create_indicators(data: Vec<f64>) -> Indicators {
    enum_map! {
        ...
        Indicator::StochasticOscillator => get_rsi_percents(&data, 1. / 14.),
        Indicator::MACDDiff => get_macd(&data),
        Indicator::WilliamsPercentRange => get_rsi_percents(&data, 1. / 14.),
    }
}
