    ...
    for generation in 0..1000 {
        ...
        for (ticker_index, bars) in mapped_historical.iter().enumerate() {
            ...
            for _ in env.current_step..prices.len() - 1 {
                ...
                let (next_obs, reward) = env.learn_step(action as usize, &actions, &mut gen_history, ticker_index);
                
                buffer.add(
                    obs.shallow_clone(),
                    action,
                    reward, // Will be updated with actual reward
                    value.double_value(&[]),
        ...
        for _ in 0..3 {
            ...
            let ratio: Tensor = ((&log_probs - &old_log_probs) as Tensor).exp();
