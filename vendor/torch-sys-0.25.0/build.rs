...
const TORCH_VERSION: &str = "2.12.1";
...
impl SystemInfo {
    ...
    fn link_cuda_runtime(&self) {
        ...
        if let Ok(entries) = fs::read_dir(&self.libtorch_lib_dir) {
        ...
        for key in ["CUDA_HOME", "CUDA_PATH"] {
            if let Ok(path) = env_var_rerun(key) {
                ...
                candidates.push(root.join("targets/x86_64-linux/lib"));
            }
        }
        candidates.extend([
        ...
        for library_dir in candidates {
            if library_dir.join("libcudart.so").exists() {
                println!("cargo:rustc-link-search=native={}", library_dir.display());
                println!("cargo:rustc-link-lib=dylib=cudart");
                return;
            }
        }
        panic!("CUDA-enabled libtorch requires libcudart, but no CUDA runtime library was found");
    }
}
