...
impl RunDir {
    pub fn create_fresh(runs_path: &str, name: Option<&str>) -> Result<Self> {
        ...
        symlink(&dir_name, &latest)
            .context("failed to create latest symlink")?;

        Ok(Self { root, gens, weights, log_file })
    }

    pub fn from_weights_path(path: &Path) -> Result<Self> {
        // path: runs/{name}/weights/{file}.ot
        let weights_dir = path.parent()
            .context("weights path has no parent")?;
        let root = weights_dir.parent()
            .context("weights dir has no parent")?;
        ...
        if !root.is_dir() { bail!("run root does not exist: {}", root.display()); }
        if !gens.is_dir() { bail!("gens dir does not exist: {}", gens.display()); }
        if !weights.is_dir() { bail!("weights dir does not exist: {}", weights.display()); }
    ...
    pub fn latest(runs_path: &str) -> Result<Self> {
        ...
        if !root.is_dir() { bail!("latest run dir does not exist: {}", root.display()); }
