...
def arm_specs(suite, selected):
    ...
    return [
        {"name": label, "jepa_mode": modes.get(label, "off"),
         "recipe": "full-none" if label in ("full-none-forecast", "conditional-full-none", *MOMENT_WEIGHTS, *READER_PLACEMENTS) else "decoupled-lattice",
         **dict(zip(MOMENT_FIELDS, MOMENT_WEIGHTS.get(label, (0.0, 0.0)))),
         **({"reader_norm": "none", "sigreg_placement": READER_PLACEMENTS[label]}
            if label in READER_PLACEMENTS else READER_DEFAULTS)}
...
def reader_settings(arm):
    ...
    expected = ({"reader_norm": "none", "sigreg_placement": READER_PLACEMENTS[arm["name"]]}
                if arm["name"] in READER_PLACEMENTS else READER_DEFAULTS)
    if values != expected:
...
def main():
    ...
    planning.add_argument("--select", action="append", choices=sorted({name for suite in SUITES.values() for name in suite}), help="repeat to submit selected suite members; temporal-reader-sigreg always requires all four fresh reader arms")
    planning.add_argument("--reference-plan", type=Path, help="reuse completed matched arms and their pinned runtime environment; an inherited RMS control never replaces fresh reader-none")
