...
def arm_specs(suite, selected):
    ...
    return [
        {"name": label, "jepa_mode": modes.get(label, "off"),
         "recipe": "full-none" if label == "full-none-forecast" else "decoupled-lattice"}
...
def recipe_for(plan, arm):
    ...
    if recipe not in RECIPES or (recipe != "decoupled-lattice" and arm["jepa_mode"] != "off"):
...
def plan(args):
    ...
    if reference:
        ...
        if expected != read_json(reference["assets"]["data_contract"]["path"]):
            raise ValueError("reference complete corpus/data contract differs")
        existing = {(arm["jepa_mode"], recipe_for(reference, arm)) for arm in reference["arms"]}
    ...
    if reference:
        baseline_present |= any(arm["jepa_mode"] == "off" and recipe_for(reference, arm) == "decoupled-lattice" for arm in reference["arms"])
    ...
    if reference:
        print(f"Reuse {len(reference['arms'])} completed reference endpoints, no retraining: {reference_record['path']}")
...
def endpoint_evidence(plan, arm):
    ...
    for key, value in RECIPES[recipe].items():
        if model.get(key.replace("-", "_")) != value:
...
def collect(args):
    ...
    failure = None
    try:
        sources = [(path, manifest, digest)]
        if manifest["reference"]:
            recorded = manifest["reference"]
            reference, actual = completed_reference(Path(recorded["path"]))
            if actual != recorded:
                raise ValueError("completed reference plan or lifecycle receipts changed")
            check_matched_reference(manifest, reference)
            sources.insert(0, (Path(recorded["path"]), reference, recorded["plan_sha256"]))
...
def main():
    ...
    planning.add_argument("--select", action="append", choices=SUITES["matched-comparison"], help="repeat to submit only selected suite members, e.g. full-none-forecast")
